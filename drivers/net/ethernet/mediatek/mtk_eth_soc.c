...
static void mtk_gmac0_rgmii_adjust(struct mtk_eth *eth,
				   phy_interface_t interface)
{
	u32 clk_rate, intf_mode, trgmii_clk_sel, rck, tck, clkcfg0;
	int ret;
	...
	tck = TXC_INV | TXCTL_DMWTLAT(2);
	rck = RXC_DQSISEL | RXCTL_DMWTLAT(2);
...
{
	/* only the 1st GMAC supports TRGMII */
	if (mac->id)
		return 0;

	if (MTK_HAS_CAPS(mac->hw->soc->caps, MTK_TRGMII_MT7621_CLK))
		return mt7621_gmac0_rgmii_adjust(mac->hw, interface);

	mtk_gmac0_rgmii_adjust(mac->hw, interface);
	return 0;
}
